...
simuler_trajectoires_adultes_2 <- function(matrice, n_iterations, n_annees = 100) {
  ...
  for (i in 1:n_iterations) {
    ...
    mat_surv <- survie_stochastique_2(matrice, n_annees)
    mat_stas <- stase_stochastique_2(matrice,n_annees)
    pop <- calculer_structure_initiale_2(matrice, 1000)
  
    
    historique[i, 1] <- sum(pop[3:4])
...
hist_null <- simuler_trajectoires_adultes_2(null_pred3,  5000)
hist_end  <- simuler_trajectoires_adultes_2(endangered3, 5000)
hist_meas <- simuler_trajectoires_adultes_2(measured3,   5000)
