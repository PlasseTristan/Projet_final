fig_2_opti_3 <- function(matrice, n_iterations, n_annees, n_ini, stockage, nom_scenario) {
  n_stades <- 3
  n_larves_supp <- stockage[1]
  n_fry_supp <- stockage[2]
  
  # 1. Structure initiale (1000 adultes répartis entre stades 3 et 4)
  pop_init <- calculer_structure_initiale_3(matrice, n_ini)
  ...
  for (i in 1:n_iterations) {
    ...
    mat_surv <- survie_stochastique_3(matrice, n_annees) # Passage G
    mat_stas <- stase_stochastique_3(matrice, n_annees)  # Stase P
...
graph_fig2_3 <- function(d1, d2, d3) {
  
  # On crée une fonction interne pour éviter de répéter 3 fois le même style
  style_fig2_3 <- function(data, label, show_x = FALSE, show_legend = FALSE) {
  ...
  graph_a <- style_fig2_3(d1, "(a)", show_legend = TRUE)
  graph_b <- style_fig2_3(d2, "(b)")
  graph_c <- style_fig2_3(d3, "(c)", show_x = TRUE)
