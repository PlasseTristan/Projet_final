...
#Taux de survie de la pop measured
s_rates <- c(
...
#Nombre de larves et de fry stockées annuellement 
n_larve <-  231185
n_fry <-  14868

#Calculer la pop initiale avec 2000 individus
pop<- calculer_structure_initiale(measured, 1000)

#_______________________________________________________________________________  
##Fonction de base pour la fig 4
  simulate_point <- function(stocking_val, stage_to_stock, n_annees = 100) {
    ...
    # Survie combinée des stades intra-annuels 
...
#_______________________________________________________________________________
##Appliquer la fonction de base avec les valeurs de la grille pour chaque stade
  
#data frame pour stocker les valeurs
...
#_______________________________________________________________________________
##Mise en forme et trillage des données
...
df_long <- pivot_longer(df_plot,
  ...
  mutate(Stocking_Stage = factor(Stocking_Stage,
    ...
    #Étiquettes positionnées à x fixe pour chaque stade
label_data <- data.frame(
  Stocking_Stage = factor(c("Larva", "Fry", "Age 1", "Age 10"),
                            levels = c("Larva", "Fry", "Age 1", "Age 10")),
  Stocking       = c(1e8,   1e6,   1e4,   2e2),   # Position X de chaque étiquettes
  Abundance_Mature = c(5400, 5400, 5400, 5400),    # Toutes au dessus des courbes
  label          = c("Larva\nstocking", "Fry\nstocking",
                       "Age 1\nstocking", "Age 10\nstocking")
  )
