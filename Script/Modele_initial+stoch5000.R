...
### 2. Boucle des 50 simulations ###
for (s in 1:n_simulations) {
  
  # Générer l'aléa propre à CETTE simulation
  ...
  pop_init <- calculer_structure_initiale(null_pred, 2000)
  pop_init[1:3] <- 0 
  sim_temp[, 1] <- pop_init
  
  # Stocker les matures à t=0
  resultats_matures[1, s] <- sum(sim_temp[13:33, 1])
