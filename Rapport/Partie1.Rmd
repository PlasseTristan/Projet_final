...
# Calculs
null_nul     <- calculer_scenario(null_pred, 5000, 100, 2000, stockage_nul, "No stocking")
null_present <- calculer_scenario(null_pred, 5000, 100, 2000, stockage_present, "Actual stocking")
null_propose <- calculer_scenario(null_pred, 5000, 100, 2000, stockage_propose, "Theoretical stocking")

end_nul     <- calculer_scenario(endangered, 5000, 100, 2000, stockage_nul, "No stocking")
end_present <- calculer_scenario(endangered, 5000, 100, 2000, stockage_present, "Actual stocking")
end_propose <- calculer_scenario(endangered, 5000, 100, 2000, stockage_propose, "Theoretical stocking")

meas_nul     <- calculer_scenario(measured, 5000, 100, 2000, stockage_nul, "No stocking")
meas_present <- calculer_scenario(measured, 5000, 100, 2000, stockage_present, "Actual stocking")
meas_propose <- calculer_scenario(measured, 5000, 100, 2000, stockage_propose, "Theoretical stocking")
...
# Sécurité log (on remplace 0 par 0.1 pour pas faire planter le log)
df_null <- df_null %>% mutate(across(c(Mediane, Inf_95, Sup_95), ~ifelse(.x <= 0.1, 0.1, .x)))
df_end <- df_end %>% mutate(across(c(Mediane, Inf_95, Sup_95), ~ifelse(.x <= 0.1, 0.1, .x)))
df_meas <- df_meas %>% mutate(across(c(Mediane, Inf_95, Sup_95), ~ifelse(.x <= 0.1, 0.1, .x)))
```
...
graph_end<-ggplot(df_end, aes(x = Annee, y = Mediane, color = Scenario, fill = Scenario)) +
  ...
  theme(
    legend.position = "none", # Légende à l'intérieur (x, y)
    legend.background = element_blank(),
    axis.title = element_text(size = 14),
    axis.text.y = element_text(size = 12, color = "black"),
    axis.text.x = element_blank(),
