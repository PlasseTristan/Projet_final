...
Chaque ligne correspond à une année distincte et chaque colonne correspond au stade associé.
...
##Fonction de base pour la fig 4
  simulate_point <- function(stocking_val, stage_to_stock, n_annees = 100) {
    ...
    # Survie_m = vecteurs contenant les survies de tous les stades de la matrice measured
    s_larve <- survie_m[2] * survie_m[3] *survie_m[4]  # Larve → stade 4
    s_fry   <- survie_m[3] * survie_m[4]               # Fry   → stade 4
...
```{r}
generer_donnees_fig4 <- function(matrice_choisie, n_iterations = 100) {
  
  # 1. Paramètres fixes et survies extraites de la matrice
  n_larve_base <- 231185
  n_fry_base   <- 14868
  n_stades     <- nrow(matrice_choisie)
  ...
  s_larve_to_4 <- s_stades[2] * s_stades[3]
  s_fry_to_4   <- s_stades[3]
  
  # 2. Définition de la sous-fonction de simulation
  # Elle est définie à l'intérieur pour isoler le calcul
  simulate_point <- function(stocking_val, stage_to_stock) {
    # On recalcule la structure initiale à chaque point pour la stabilité
    pop <- calculer_structure_initiale(matrice_choisie, 1000)
    
    for (t in 1:100) {
      pop <- as.vector(matrice_choisie %*% pop)
      pop[1:3] <- 0 # Recensement annuel
      
      if (stage_to_stock == "Larva") {
        pop[4] <- pop[4] + ((stocking_val + n_larve_base) * s_larve_to_4)
      } else if (stage_to_stock == "Fry") {
        pop[4] <- pop[4] + ((stocking_val + n_fry_base) * s_fry_to_4)
      } else if (stage_to_stock == "Age 1") {
        pop[4] <- pop[4] + stocking_val
      } else if (stage_to_stock == "Age 10") {
        if (t > 10) pop[13] <- pop[13] + stocking_val
      ...
      if (any(is.na(pop)) || sum(pop) > 1e16) return(NA)
    }
    return(sum(pop[13:33]))
  }
  
  # 3. Création de la grille d'efforts
  efforts   <- seq(0, 9, length.out = 1000)
  grid_vals <- 10^efforts
  stades    <- c("Larva", "Fry", "Age 1", "Age 10")
  
  # 4. Calcul et transformation des données (Tidyverse)
...
df_long_clean <- generer_donnees_fig4(measured)
```
